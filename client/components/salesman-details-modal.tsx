...
import { getSalesmanDetails, type SalesmanDetails } from '@/utils/api';
...
export default function SalesmanDetailsModal({ visible, onClose, salesmanId }: SalesmanDetailsModalProps) {
  const { token } = useAuthStore();
  const [loading, setLoading] = useState(false);
  const [salesman, setSalesman] = useState<SalesmanDetails | null>(null);

  useEffect(() => {
    if (visible && salesmanId && token) {
      loadSalesmanDetails();
    } else {
      setSalesman(null);
    }
  }, [visible, salesmanId, token]);

  const loadSalesmanDetails = async () => {
    if (!salesmanId || !token) return;

    try {
      setLoading(true);
      const response = await getSalesmanDetails(salesmanId, token);
      if (response.success) {
        setSalesman(response.salesman);
      }
    } catch (error) {
      console.error('Failed to load salesman details:', error);
      Alert.alert('Error', 'Failed to load salesman details. Please try again.');
      onClose();
    } finally {
      setLoading(false);
    }
  ...
  const copyPassword = async () => {
    if (salesman?.plainPassword) {
      if (Platform.OS === 'web' && navigator.clipboard) {
        await navigator.clipboard.writeText(salesman.plainPassword);
        Alert.alert('Success', 'Password copied to clipboard!');
      } else {
        Alert.alert('Password', salesman.plainPassword, [{ text: 'OK' }]);
      }
    }
  };
    ...
    <Modal
      ...
      <SafeAreaView className="flex-1 bg-[#1a1f3a]">
        ...
        <View className="flex-row items-center justify-between px-6 py-4 border-b border-[#2d324a]">
          <Text className="text-white text-xl font-bold">Salesman Details</Text>
          ...
          <ScrollView
            ...
            <View className="px-6 py-6">
              ...
              <View className="items-center mb-6">
                ...
                <Text className="text-white text-2xl font-bold mb-1">{salesman.name}</Text>
                <View
              ...
              <View className="mb-6">
                ...
                <DetailRow label="Full Name" value={salesman.name} />
                <DetailRow label="Email" value={salesman.email} />
                <DetailRow label="Phone" value={salesman.phone || 'Not provided'} />
              </View>

              <View className="mb-6">
                <Text className="text-white text-lg font-bold mb-4">Work Information</Text>
                <DetailRow label="Salesman ID" value={salesman.id.toString()} />
                <DetailRow label="Shop Assignment" value={salesman.branch} />
                <DetailRow label="Region" value={salesman.region || 'Not assigned'} />
                <DetailRow label="Status" value={salesman.status} />
              </View>
              ...
              <View className="mb-6">
                ...
                <DetailRow label="Created At" value={formatDate(salesman.createdAt)} />
                {salesman.updatedAt && (
                  <DetailRow label="Last Updated" value={formatDate(salesman.updatedAt)} />
                )}
              </View>
            </View>
          </ScrollView>
